...
# 새로운 콘텐츠 생성 후 DB 저장
def create_content(db: Session, payload: schemas.ContentBase) -> models.Content:
...
def get_content_by_cid(db: Session, cid: UUID) -> models.Content | None:
    return db.query(models.Content).filter(models.Content.id == cid).first()

# 콘텐츠 선택적 수정 및 반영
def update_content(db: Session, cid: UUID, payload: schemas.ContentUpdate) -> models.Content | None:
    query = db.query(models.Content).filter(models.Content.id == cid)
